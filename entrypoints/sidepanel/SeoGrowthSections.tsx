    ...
    <>
      {mode === 'summary' ? <section className="plain-section page-context-section" aria-labelledby="page-context-title">
        ...
        <div className="context-grid">
          ...
          <SelectField label="这是什么页面" value={context.pageType} onChange={(value) => setContext((current) => ({ ...current, pageType: value as AuditContext['pageType'] }))} options={[{ value: 'auto', label: '让插件自动判断' }, { value: 'home', label: '首页' }, { value: 'article', label: '文章 / 教程' }, { value: 'product_service', label: '产品 / 服务' }, { value: 'category', label: '分类 / 列表' }, { value: 'tool', label: '公开工具页' }, { value: 'search_filter', label: '搜索 / 筛选页' }, { value: 'internal_app', label: '登录后工具 / 内部页' }]} />
