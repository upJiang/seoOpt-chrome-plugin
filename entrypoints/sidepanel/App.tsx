...
function reportToMarkdown(report: AuditReport): string {
  ...
  if (report.snapshot.overseas) {
    ...
    lines.push(
      '## 海外站优化',
      '',
      `- 已确认正常 / 已确认问题 / 优化建议：${overseasDiagnosis?.normalCount ?? 0} / ${overseasDiagnosis?.issueCount ?? 0} / ${overseasDiagnosis?.opportunityCount ?? 0}`,
      `- 海外市场判断：${overseasDiagnosis?.marketAssessment.headline ?? '证据不足'}`,
      `- 判断摘要：${overseasDiagnosis?.marketAssessment.summary ?? '当前页面没有可用海外证据。'}`,
      ...
      '### 海外市场能力',
      '',
      ...(overseasDiagnosis?.marketAssessment.capabilities.map((item) => `- ${item.label}：${item.state}。${item.conclusion} 证据：${item.evidence} 限制：${item.limitation}`) ?? ['- 暂无可用能力判断']),
      '',
      '### 已确认问题',
      '',
